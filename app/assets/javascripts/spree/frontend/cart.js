window.addEventListener('DOMContentLoaded', () => {
  ...
  if (cartForm) {
    const deleteButtons = cartForm.querySelectorAll('[data-js-cart-item-delete-button] input')

    deleteButtons.forEach((deleteButton) => {
      deleteButton.addEventListener('click', () => {
        const lineItem = deleteButton.parentNode.parentNode
        lineItem.querySelector('[data-js-cart-item-quantity] input').setAttribute('value', 0)
      })
    })
